function [pap_anv_He]=paporbitdataHe(Daydata,pap_Select_data,orbit)
...
for i=1:a 
    ...
    for j=1:length(pap_Select_data)          
        ...
        if (pap_Select_data(j,10)>= Daydata(i,10)-2) && (pap_Select_data(j,10)<= Daydata(i,10)+2) && (pap_Select_data(j,11)>=Daydata(i,11)-1) && (pap_Select_data(j,11)<=Daydata(i,11)+1 && pap_Select_data(j,13) ~= 0)&&(pap_Select_data(j,2)==orbit)
            ct = ct + 1;
            pap_sum_He=pap_sum_He+pap_Select_data(j,13);
        end
          pap_anv_He(i) = pap_sum_He/ ct;  
    end
end
end
