...
function [pap_observation_data,value_H_day,value_He_day,value_O_day,value_Ti_day]=papdailydata10(month,day,orbit,pap_Select_data)
pap_observation_data = [];
 [q,~] = size(pap_Select_data);
 
 for g = 1:q
     
        if (pap_Select_data(g,4) == month) && (pap_Select_data(g,5) == day && pap_Select_data(g,2) == orbit && pap_Select_data(g,10)>=110 && pap_Select_data(g,10)<=138 ) 
