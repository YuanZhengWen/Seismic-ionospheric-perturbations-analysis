...
function [observation_data,value_Ne_day,value_Te_day]=dailydata(month,day,orbit,Select_data)
observation_data = [];
[q,~] = size(Select_data);
 
 for g = 1:q
     
        if (Select_data(g,4) == month) && (Select_data(g,5) == day && Select_data(g,2) == orbit && Select_data(g,6)>=118 && Select_data(g,6)<=141 ) 
